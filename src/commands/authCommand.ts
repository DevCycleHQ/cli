...
export default abstract class AuthCommand extends Base {
    static flags = {
        ...
        org: Flags.string({
            description:
                'The `name` of the org to sign in as (not the `display_name`)',
        }),
    }
    ...
    public async retrieveProjectFromConfig(
        ...
        const savedProject =
            this.repoConfig?.project || this.userConfig?.project
        if (savedProject) {
            const matchingProject = projects.find(
                (project) => project.key === savedProject,
            )
            if (matchingProject) {
    ...
    private async retrieveProject(projects: Project[]): Promise<Project> {
        ...
        if (flags.project) {
            ...
            return (
                (await this.retrieveProjectFromConfig(projects)) ||
                promptForProject(projects)
            )
        }
    }
    ...
    public async projectFromFlag(projects: Project[]) {
        const { flags } = await this.parse(AuthCommand)
        const matchingProject = projects.find(
            (project) => project.key === flags.project,
        )
        if (!matchingProject) {
            throw new Error(
                `There is no project with the key ${flags.project} in this organization`,
            )
        }
        return matchingProject
    }

    public async retrieveOrganizationFromConfig(): Promise<
    ...
    private async retrieveOrganization(
        ...
        if (flags.org) {
            const matchingOrg = organizations.find(
                (org) => org.name === flags.org,
            )
            if (!matchingOrg) {
                throw new Error(
                    `You are not a member of an org with the name ${flags.org}`,
                )
            }
            return matchingOrg
        } else {
