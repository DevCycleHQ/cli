    ...
    public async run(): Promise<void> {
        ...
        if (!flags.headless && !params.featureKey) {
            ...
            const responses = await inquirer.prompt<
                FeaturePromptResult & EnvironmentPromptResult
            >(this.prompts, {
        ...
        if (flags.headless) {
            ...
            const environments = params.environment
                ? [params.environment]
                : await fetchEnvironments(this.authToken, this.projectKey)
            const variations =
                params.feature?.variations ||
            ...
            renderTargetingTree(targeting, environments, variations, audiences)
        }
    }
}
