...
export const executeDiff = (
    ...
    try {
        let files = ''
        if (codeInsightsConfig?.includeFiles || codeInsightsConfig?.excludeFiles || flags.include || flags.exclude) {
            files = ' ' + getFilteredFiles(flags, codeInsightsConfig).join(' ')
        }

        execSync(`git diff ${diffCommand}${files} > diff.txt`, { stdio: 'ignore' })
