...
export abstract class BaseParser {
  ...
  private buildRegexPattern() {
    ...
    return new RegExp(
      clientNamePattern + this.variableMethodPattern.source + parameterPattern,
    );
  }
  ...
  private filterAndReduceLines(lines: ParsedLine[]): {
    ...
    return lines
      .filter(
        (line) => !line.isComment(this.commentCharacters) && !line.isEmpty(),
      )
      .reduce(
        (acc, line) => {
  ...
  parseFile(file: usage.File): VariableUsageMatch[] {
    ...
    matches.forEach((match) => {
      ...
      const bufferedContent = file.lines
        .filter(
          (line) =>
            range.start - buffer <= line.ln && range.end + buffer >= line.ln,
        )
