...
export abstract class RefactorEngine {
      ...
      enter: function (node) {
        ...
        if (
          ...
          const valueLiteral = objectProperties.find(
            (prop) => (prop.key as Identifier).name === propertyName,
          )?.value;
          if (valueLiteral) {
            engine.changed = true;
            return valueLiteral;
        ...
        const callExpression =
          node.type === "ExpressionStatement" && getCallExpression(node);
  ...
  private printAST = () => {
    try {
      ...
      console.warn(
        chalk.yellow(`Error occurred while recasting file ${this.filepath}`),
      );
      console.warn(`\t${err.message}`);
    }
  };
