...
export async function listEnvironmentsHandler(
    ...
    return await apiClient.executeWithDashboardLink(
        ...
        async (authToken: string, projectKey: string | undefined) => {
            if (!projectKey) {
                throw new Error(
                    'Project key is required for listing environments. Please select a project using the select_project tool first.',
                )
            }
...
export async function getSdkKeysHandler(
    ...
    return await apiClient.executeWithDashboardLink(
        ...
        async (authToken: string, projectKey: string | undefined) => {
            if (!projectKey) {
                throw new Error(
                    'Project key is required for getting SDK keys. Please select a project using the select_project tool first.',
                )
            }
...
export async function createEnvironmentHandler(
    ...
    return await apiClient.executeWithDashboardLink(
        ...
        async (authToken: string, projectKey: string | undefined) => {
            if (!projectKey) {
                throw new Error(
                    'Project key is required for creating environments. Please select a project using the select_project tool first.',
                )
            }
            return await handleZodiosValidationErrors(
...
export async function updateEnvironmentHandler(
    ...
    return await apiClient.executeWithDashboardLink(
        ...
        async (authToken: string, projectKey: string | undefined) => {
            if (!projectKey) {
                throw new Error(
                    'Project key is required for updating environments. Please select a project using the select_project tool first.',
                )
            }
            return await handleZodiosValidationErrors(
                () =>
