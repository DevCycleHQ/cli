...
/**
 * Set up a persistent nock interceptor for the auth endpoint.
 * This is called at the start of each dvcTest() chain to ensure
 * the auth mock is available, since fancy-test's nock plugin calls
 * nock.cleanAll() after each test which destroys all interceptors.
 */
function setupAuthNock() {
    Nock(AUTH_URL).persist().post('/oauth/token').reply(200, {
